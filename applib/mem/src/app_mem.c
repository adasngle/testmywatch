...
static void traverse_link_list(
    applib_mem_header_struct *list, list_traverse_handler callback, void *arg1, void *arg2, void *arg3);
...
static kal_bool print_flmm_memory_block(KAL_FLMM_ID adm_id, kal_vm_meminfo_t *info, void *v_addr, void *userdata)
{
    ...
    if (info->slmm == NULL)
    {
        ...
        if (p->guard_pattern[0] != APPLIB_MEM_HEADER_PATTERN1)
        {
    ...
    )
{
    ...
    chunk = header + 1;
    footer = (applib_mem_footer_struct*) (((char*)chunk) + APPMEM_ALIGN_TO_N(mem_size, 4));

    /* header */
    header->owner_id = owner_id;
    header->chunk_size = mem_size;
    header->flag = flag;
//	ASM_ASSERT(header->guard_pattern[0] != APPLIB_MEM_HEADER_PATTERN1);
...

{
    ...
    footer = (applib_mem_footer_struct*) (((char*)mem_ptr) + APPMEM_ALIGN_TO_N(header->chunk_size, 4));
    
    ASM_ASSERT(APPLIB_MEM_COMP_PATTERN(header->guard_pattern, APPLIB_MEM_HEADER_PATTERN1) && 
...
static kal_uint32 calc_real_alloc_size(kal_uint32 mem_size, kal_uint32 align_size)
{
    ...
    chunk_size = APPMEM_ALIGN_TO_N(
        mem_size + sizeof(applib_mem_header_struct) + sizeof(applib_mem_footer_struct),
        align_size);
    return chunk_size;
}
...
static kal_bool is_valid_block(void *mem_ptr, kal_uint32 align_size)
{
    ...
    if (
        #if defined __MMI_USE_MMV2__
        (header->flag & APPLIB_MEM_HEADER_FLAG_IS_SLMM) == 0 &&
        #endif
    ...
    {
        #if defined __MMI_USE_MMV2__
        if ((header->flag & APPLIB_MEM_HEADER_FLAG_IS_SLMM) != 0)
            return KAL_TRUE;
        #endif
        if ( APPLIB_MEM_COMP_PATTERN(footer->guard_pattern, APPLIB_MEM_FOOTER_PATTERN1))
            return KAL_TRUE;
    }
     return KAL_FALSE;
}
    ...
    void *arg3)
{
	...
	if (cb != NULL && (MMI_TRUE == cb(mmi_frm_get_root_app(node->owner_id))))
	{
	    return KAL_FALSE;
	}
    
    if (inc_hide == KAL_FALSE && is_app_visible(node) == KAL_FALSE)
    {
        return KAL_FALSE;
    }
...
kal_uint32 applib_mem_ap_get_single_usage(kal_uint32 app_id)
{
    ...
    APP_ASM_LOCK;
    traverse_link_list(
        g_applib_mem_cntx.app_head.next,
        get_app_usage_handler, 
        (void *)root_app_id,
        (void *)&usage,
        NULL);
...
kal_uint32 applib_mem_ap_get_current_usage(applib_mem_ap_usage_struct *applist, kal_uint32 max_items)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    return applib_mem_ap_get_current_usage_ex(applist, max_items, KAL_FALSE, KAL_FALSE, NULL, 0, NULL);
}
 ...
 *  applist:     [OUT]    List of applications and its memory consumption
 *  max_items:   [IN]     The maximum size of 'applist'
 *  inc_hide:    [IN]     include hided app or not
 *  skip_v10:    [IN]     skip ASM1.0 app or not
 *  skip_list:   [IN]     the app id list need to skip
 *  skip_count:  [IN]     the app id list length
 * RETURNS
 *  the number of applications listed
 *****************************************************************************/
kal_uint32 applib_mem_ap_get_current_usage_ex(applib_mem_ap_usage_struct *applist, kal_uint32 max_items, kal_bool inc_hide, kal_bool skip_v10, kal_uint16 *skiplist, U32 skip_count, app_get_usage_check_cb cb)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_uint32 applib_mem_ap_get_max_alloc_size_int(void)
{
    ...
    #endif
    overhead = sizeof(applib_mem_header_struct) + sizeof(applib_mem_footer_struct) + 31 * 2;
    if (result <= overhead)
    {
        result = 0;
    }
...
static void test_set_alloc_free_info(void)
{
    ...
    for (i = 4; i >= 0; i--)
    {
        ...
        ASM_ASSERT(APPLIB_MEM_COMP_PATTERN(header->guard_pattern, APPLIB_MEM_HEADER_PATTERN2));
        mmi_mfree(chunk[i]);
