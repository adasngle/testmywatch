...
/*
 * terrence,2013/07/03,bt pairing auto accept support,means fixed PIN code or just work,such as BT headset
 * Variables setting by custom
 */

/*
 * bt_pairing_auto_accept_support
 *
 * KAL_TRUE: bt device has not UI to show PIN/Number for pairing,and auto accept/confirm,such as bt headset.
 * KAL_FALSE:bt device has UI to show PIN/Number for pairing,and need user click to continue,such as phone,MID,PC etc.
 * default value is KAL_FALSE,custom can change it according to above rules.
 */
 
kal_bool bt_pairing_auto_accept_support = KAL_FALSE;

kal_uint8 bt_fixed_pin_code[4]={0x30,0x30,0x30,0x30}; //pin code:0000
...
/*
 * bt_dialer_sniff_interval_weight
 *
 * if disable low power feature,we will ignore this parameter.
 * if interval_weight == 0,we will igore this patameter
 * sniff interval = ( supervision_timeout  / interval_weight)
 * default value is 10,custom can change it
 */

kal_uint16 bt_dialer_sniff_interval_weight = 10;
...
// This part is used for Non-BT MT6236 projects.
    ...
    #if defined(__BTMODULE_MT6236__) || defined(__BTMODULE_MT6276__) || defined(__BTMODULE_MT6256__) || defined(__BTMODULE_MT6255__) || defined(__BTMODULE_MT6250__) || defined(__BTMODULE_MT6260__)
...
{
    return bt_pairing_auto_accept_support;
}
/*
 * terrence,2013/06/26,BT dialer feature:Low power solution
 * API used by MTK
...
kal_uint16 Custom_BTDialer_SniffInterval_Weight(void)
{
   return bt_dialer_sniff_interval_weight;
}
