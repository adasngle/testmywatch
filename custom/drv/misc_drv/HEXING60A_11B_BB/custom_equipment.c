...
/*GPIO_LABEL_LED_LCD */      {GPIO_VAILD, GPIO_PORT_5, netname[GPIO_PORT_5], NULL },
...
kal_bool custom_cfg_gpio_set_level(kal_uint8 gpio_dev_type, kal_uint8 gpio_dev_level )
{
  ...
  #if defined(__BOOST_LCD_OFF__)
    // With LCD condition
    pmu6253_bl_enable(KAL_FALSE);
    pmu6253_vboost_enable(KAL_FALSE);
  #else
    // With LCD condition
    pmu6253_bl_enable(KAL_TRUE);
    pmu6253_vboost_enable(KAL_TRUE);
  #endif // __BOOST_LCD_OFF__
