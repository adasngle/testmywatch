...
extern kal_uint32 Load$$VIVA$$Base;
#define VIVA_START_ADDR &Load$$VIVA$$Base;
#endif
...
static void NFB_SV5_startLoadingSecondaryMAUI(void)
{
   ...
   kal_uint32 pageBuffer = (kal_uint32)(&Image$$INTSRAM_MULTIMEDIA$$Base) + 0x1000;
#endif /* MT6260 */
   DCL_HANDLE handle;
...
#if defined(MT6260)
   pageBuffer = (kal_uint32*)kal_tmp_mem_alloc(pagesize);
#endif /* MT6260 */
   while ( index<flashLayoutPtr->regionCount )
   {
...
#endif      
      {
          ...
          {
#if defined(MT6260)
              ret = ftlFuncTbl.FTL_ReadPage(blockIdx, pageIdx, pageBuffer, NULL);
#else /* MT6260 */
          ...
          if( ret == FTL_SUCCESS )
          {
              if (flashLayoutPtr->region[index].binaryType != THIRD_ROM)    // debug: Stanley Chu
              {
              ...
              if(  flashLayoutPtr->region[index].binaryType ==PRIMARY_MAUI)
              {
                  ...
                  accumulativeSize = pGFHFileInfo->m_file_len;
                      ...
                      && flashLayoutPtr->region[index].binaryType != THIRD_ROM
...
#if defined(__VI_ENALBED__)
              if ( flashLayoutPtr->region[index].binaryType == VIVA )
              {
                  loadAddr[index] = (kal_uint32)VIVA_START_ADDR;
              }
              else
#endif /* __VI_ENALBED__ */
              {
   ...
   while ( index<flashLayoutPtr->regionCount )
   {
      ...
      /* Skip third ROM */
      else if (flashLayoutPtr->region[index].binaryType == THIRD_ROM)
      {
         index++;
         continue;
      }
      ...
      if( flashLayoutPtr->region[index].binaryType == VIVA )
      {
         ...
         extern void AliceNandInit(void);
         AliceNandInit();
#endif /* __AICE_ENALBED__ */
      }
#endif /* __DSP_BOOT_SEC__ */
...
#if defined(MT6260)
   Nand_ImageList_S  *imageHeaderPtr = (Nand_ImageList_S*)&(g_pass_info.m_bl_image_list);   
#else /* MT6260 */
