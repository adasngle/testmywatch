...
#include 	"intrCtrl.h"
#include	"stack_ltlcom.h"		/* for allocate_ilm & msg_send_ext_queue definitions */
#include	"stack_config.h"		/* for module_type, MOD_DRV_HISR, MOD_GPS, MOD_UEM */
#include    "drv_hisr.h"
    ...
    if (IsOSRunning()) {
        if (module_reg_ilm_flag == KAL_TRUE)
        {
            ...
            while( index < MODULE_REGISTER_ILM_MAX_NUM )
            {
                if (rtc_reg_mod_ilm_info[index].dest_mod_id != MOD_NIL)
                {
                    RTC_ilm = allocate_ilm(module_id);
                    RTC_ilm->src_mod_id = module_id;
                    RTC_ilm->dest_mod_id = rtc_reg_mod_ilm_info[index].dest_mod_id;
                    RTC_ilm->sap_id = rtc_reg_mod_ilm_info[index].sap_id;
                    RTC_ilm->msg_id = rtc_reg_mod_ilm_info[index].msg_id;
                    RTC_ilm->local_para_ptr = (local_para_struct *)rtc_ind;
...
/* under construction !*/
	...
	if (status & RTC_IRQ_STATUS_AL_STAT)
	{
		RTC_CALLBAC.rtc_alfunc();	
     	#if defined(DRV_RTC_SET_AUTO_AFTER_ALARM)
...
#if !defined(__FUE__) && !defined(__UBL__)
#if defined(DRV_RTC_EXTEND_TO_KEEP_RTC_TIME)
#if defined(DRV_RTC_LOW_POWER_DETECT)
    if (RTC_is_Extend_To_Keep_RTC_Time())
    {
        LPD = 0;
    }
#endif //#if defined(DRV_RTC_LOW_POWER_DETECT)
...
#endif //#if defined(DRV_RTC_INTERNAL_32K_SWITCH_FAIL_DETECT)
#endif //#if defined(DRV_RTC_EXTEND_TO_KEEP_RTC_TIME)
#endif //#if !defined(__FUE__) && !defined(__UBL__)
...
void writeXOSCCAL(kal_uint16 reg_value)
{
    ...
    DRV_RTC_WriteReg(RTC_XOSCCAL, reg_value);
...
void send_DRVUEM_RTC_PERIOD_IND_msg(kal_uint8 rtc_ind)
{
    ilm_struct *RTC_ilm;
	...
	RTC_Prim->rtc_ind = rtc_ind;//RTC_TC_IND;
	...
	RTC_ilm->peer_buff_ptr = NULL;
    /* lint -e(534) */msg_send_ext_queue(RTC_ilm);
}
#endif
...
void RTC_TCintr(void)
{
	...
	kal_bool need_notify = KAL_FALSE;
	...
	if (cali_state != (kal_uint8)CAL_STATE_NONE)
	{
		...
		RTC_GetTime_(&current_time);
		if (previous_time.rtc_min != current_time.rtc_min/*current_time.rtc_sec == 0*/)
		{
			if (cali_state == (kal_uint8)CAL_STATE_INIT)
			{
...
/* under construction !*/
#else
				if (previous_time.rtc_min != current_time.rtc_min);
					need_notify = KAL_TRUE;
#endif
				#endif // #if defined(SLEEP_WORKAROUND_IN_RTC)
				break;
...
/* under construction !*/
#else
				if (previous_time.rtc_hour != current_time.rtc_hour);
					need_notify = KAL_TRUE;
#endif
				#endif // #if defined(SLEEP_WORKAROUND_IN_RTC)
				break;
...
/* under construction !*/
#else
				if (previous_time.rtc_day != current_time.rtc_day);
					need_notify = KAL_TRUE;
#endif
				#endif // #if defined(SLEEP_WORKAROUND_IN_RTC)
				break;
...
/* under construction !*/
#else
				if (previous_time.rtc_mon != current_time.rtc_mon);
					need_notify = KAL_TRUE;
#endif
				#endif // #if defined(SLEEP_WORKAROUND_IN_RTC)
				break;
...
/* under construction !*/
#else
				if (previous_time.rtc_year != current_time.rtc_year);
					need_notify = KAL_TRUE;
#endif
				#endif // #if defined(SLEEP_WORKAROUND_IN_RTC)
				break;
  	...
  	kal_prompt_trace(MOD_RTC_HISR, "TC interrupt for user\r\n");
   	#endif /* RTC_CAL_DEBUG */
	drv_trace4(TRACE_GROUP_10, RTC_TC_INTR_FOR_USER, current_time.rtc_hour, current_time.rtc_min, current_time.rtc_sec, rtc_current_frame_tick);
   	//drv_trace0(TRACE_GROUP_10, RTC_TC_INTR_FOR_USER);     
   	previous_time = current_time;
#endif /* defined(RTC_HW_CALIBRATION) || defined(RTC_SW_CALIBRATION) */

	if (KAL_TRUE == need_notify)
        send_DRVUEM_RTC_PERIOD_IND_msg(RTC_TC_IND);
...
void RTC_ALintr(void)
{
    ...
    send_DRVUEM_RTC_PERIOD_IND_msg(RTC_AL_IND);
...
void rtc_write_to_nvram(void)
{
   ...
   // kal_bool msg_b; //for avoid Lint Warning 534: Ignoring return value of function 'msg_send_ext_queue"
	...
	/* lint -e(534) */kal_mem_cpy(get_pdu_ptr(data_stream, &pdu_len), (void *)&rtc_nvram_s, sizeof(rtc_nvram_data));
	...
	/* lint -e(534) */msg_send_ext_queue(rtc_ilm_ptr);
...
void RTC_init_(void (*RTC_TCCallback)(void), void (*RTC_ALCallback)(void)) // Validate
{
	...
	*/
	...
	RTC_GetTimeOnly(&previous_time);

#if 0//defined(__MTK_INTERNAL__)&&defined(DRV_RTC_LOW_POWER_DETECT)
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
static void writeCheckRegister(kal_uint32 reg_addr, kal_uint16 reg_value)
{
    ...
    for (loop = 0; loop < 1000000; loop++)
    {
        DRV_RTC_WriteReg(reg_addr, reg_value);
        ...
        if (DRV_RTC_Reg(reg_addr) == reg_value) 
            break; 
    } 
}
...
void RTC_Set_PDN_bits(RTC_PDN_INDEX PDNIndex, kal_uint16 flag)
{
    ...
    REG_COMM |= flag;
    writeCheckRegister(reg_addr, REG_COMM);
...
void RTC_Write_PDN_bits(RTC_PDN_INDEX PDNIndex, kal_uint16 flag)
{
    ...
    writeCheckRegister(reg_addr, flag);
}
