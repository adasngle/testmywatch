...
kal_bool SD_eMMC_ECSD_configPartition(eMMC_partitions partition, kal_uint32 startAddr, kal_uint32 size, kal_bool isEnhanced)
{
	...
	*/
	...
	partitionSize[eMMC_GP_partition1 - eMMC_GP_partition1] = gSD->mCSD.ext_csd->gp_size_mult[0] + ( gSD->mCSD.ext_csd->gp_size_mult[1] * 8) | ( gSD->mCSD.ext_csd->gp_size_mult[2] * 64);
	partitionSize[eMMC_GP_partition2 - eMMC_GP_partition1] = gSD->mCSD.ext_csd->gp_size_mult[3] + ( gSD->mCSD.ext_csd->gp_size_mult[4] * 8) | ( gSD->mCSD.ext_csd->gp_size_mult[5] * 64);
	partitionSize[eMMC_GP_partition3 - eMMC_GP_partition1] = gSD->mCSD.ext_csd->gp_size_mult[6] + ( gSD->mCSD.ext_csd->gp_size_mult[7] * 8) | ( gSD->mCSD.ext_csd->gp_size_mult[8] * 64);
	partitionSize[eMMC_GP_partition4 - eMMC_GP_partition1] = gSD->mCSD.ext_csd->gp_size_mult[9] + ( gSD->mCSD.ext_csd->gp_size_mult[10] * 8) | ( gSD->mCSD.ext_csd->gp_size_mult[11] * 64);
	partitionSize[eMMC_user_Area - eMMC_GP_partition1] = gSD->mCSD.ext_csd->enh_size_mult[0] + ( gSD->mCSD.ext_csd->enh_size_mult[1] * 8) | ( gSD->mCSD.ext_csd->enh_size_mult[2] * 64);
...
kal_bool SD_eMMC_ECSD_setCurrentPart(eMMC_partitions partition)
{
	...
	if(NULL == partition || eMMC_user_Area < partition)
		ASSERT(0);

	/*if this is not eMMC 4.4, assert it */
	//EMMC44_CHECK_AND_ASSERT;

    pt = (partition == eMMC_user_Area)? (eMMC_partitions)0 : partition;
	...
	*/
	if (3 < gSD->mCSD.ext_csd->ext_csd_rev){
		if(0 != gSD->mCSD.ext_csd->boot_size_mul){ //has boot partition, this is eMMC
			gSD->emmc_info.isEmmcV44 = KAL_TRUE;
			...
			if( 0x1 == (0x1 & gSD->mCSD.ext_csd->partition_support))
				gSD->emmc_info.supportPartition = KAL_TRUE;
			else
				gSD->emmc_info.supportPartition = KAL_FALSE;

			if( 0x2 == (0x2 & gSD->mCSD.ext_csd->partition_support))
				gSD->emmc_info.supportEnhancedPart = KAL_TRUE;
			else
				gSD->emmc_info.supportEnhancedPart = KAL_FALSE;
		}
...
SDC_CMD_STATUS SD_SetMMC40_bus_high_speed(void)
{
	...
	//	kal_uint8 *pData;
		...
		if((KAL_FALSE == gMSDC_Handle->trySingleLine)&&(gSD->mCSD.ext_csd->ext_csd_rev>=5)){
		// select bus width
...
kal_uint32 SD_setCLKAndTest(kal_uint32 *targetCLK)
{
...
/* under construction !*/
#endif
			status = SD_setCLKAndTest(&testingClk);
			if( ERR_CMDOK_DATREADFAIL==status )
			{
				MSDC_STOP_TIMER();// mtk modify
			//	sprintf(buff, "[SD_tuneCLK2] -> return TRUE end!!!");//for trace
			//	kal_bootup_print(buff);//for trace
				return ERR_CMDOK_DATREADFAIL;
			}
			else if(status)
			{
				//msdc_clkTuneUpperBund = testingCLk;// mtk modify
		    	MSDC_STOP_TIMER();// mtk modify
			//	sprintf(buff, "[SD_tuneCLK2] -> return TRUE end!!!");//for trace
			//	kal_bootup_print(buff);//for trace
		//		kal_bootup_trace(TRACE_INFO,liyan_turnclk2_end,0);
				return KAL_TRUE;
            }
		}
...
SDC_CMD_STATUS SD_Initialize(void)
{
    ...
    // Temp solution for 6255E1.
    
	val.mod |= 0;
	val1.mod |=0;
#if defined(DRV_MSDC_LATCH_MT6276_SERIES)
#if !defined(__UBL__) && !defined(__FUE__)
	//pmic_adpt2_set_vmc(PMIC_ADPT_VOLT_3_3); //replaced by DCL
...
// Add by GuoXin
	...
	MSDC_SetClockSource(MSDC_CLOCK_MCU);
	#endif
		#if defined(DRV_MSDC_CLK_PAT_6260_SERIES)
...
//! patch for MT6268/6253/6516 and later chips
	...
	if (status == ERR_CMDOK_DATREADFAIL) {
		...
		return ERR_CMDOK_DATREADFAIL;// Cool Lee add
	}
	else if (status == KAL_FALSE) {
		SD_TRACE2(TRACE_GROUP_5, MSDC_ERROR_TUNECLKFAIL, MSDC_Reg32(MSDC_CFG), MSDC_Reg32(MSDC_IOCON));
	}
#endif
...
//#endif//#if defined(DRV_MSDC_CLK_MT6276_SERIES)
		...
		if(gSD->flags & SD_FLAG_CMD6_SUPPORT)
		{
			...
			if(status == NO_ERROR)
			{
				kal_print("SD_SelectHighSpeed_SD11 Success.\r\n");
...
SDC_CMD_STATUS SD_WaitCmdRdyOrTo(void)
{
	...
	do
	{
            SD_CheckTimeoutWithSleep(t1,  MSDC_TIMEOUT_PERIOD_CMD*11);
...
SDC_CMD_STATUS SD_WaitDatRdyOrTo(void)
{
	...
	do
	{
        SD_CheckTimeoutWithSleep(t1, MSDC_TIMEOUT_PERIOD_DAT * 11);
#ifndef DRV_LSD
		sdc_datsta = MSDC_Reg32(SDC_DATSTA);
#else
...
SDC_CMD_STATUS SD_WaitLastBlockFinish(void)
{
    ...
    do{
		...
		if(SD_CheckTimeoutWithSleep(t1,10))
		{
...
/* under construction !*/
			...
			if(gMSDC_Handle->dat_sta & SDC_DATSTA_DATCRCERR
#ifdef MSDC_DEFECT_WRITE_CRCERR	// WHQA_0006199 workaround for MT6236E2 write crc not reported (solution 2)
			|| (MSDC_Reg32(SDC_STA) & (SDC_STA_FEDATBUSY|SDC_STA_DATBUSY)) == SDC_STA_DATBUSY
#endif
				){     
				kal_print("[MSDC]:dat crc");
#ifdef DRV_MSDC_2_DLT_FOR_RD_WR
		...
		if(triedDLTTimes < 3){
			...
			if((status = SD_StopTrans(KAL_TRUE))!=NO_ERROR){
				kal_print("[MSDC]retry DLT but Stop failed");
				goto ERR_Exit;
			}
	...
	DisableMSDC_DMA();
#if DRV_MSDC_CHECK_MULTIBLOCK_WRITE_FINISHED	
...
retryDLT:
    ...
    else
    {
        ...
        if(status != NO_ERROR){
		#if defined(MSDC_TRACE_LEVEL3)
            MSDC_add_dbg_msg(msdc_dbg_event_SD_multi_blk_write_issue1, MSDC_Reg32(DMA_RLCT(gMSDC_Handle->msdc_dmaport)), status);
		#endif
            //sd_writeFailReason = 1;
            gMSDC_Handle->dat_sta = MSDC_Reg32(SDC_DATSTA);
#if 0	// to prevent from WHQA_0006199 happened again
