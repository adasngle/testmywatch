...
err:
	if(status != NO_ERROR)
	{
		kal_print("sdio_initialize error \r\n");
...
{
	while( MSDC_Reg32(SDC_STA) & SDC_STA_R1BSY);
}
/*************************************************************************
* FUNCTION
...
SDC_CMD_STATUS SDIO_Cmd53_isr(cmd53_config_struct *p)
{
   ...
   else/*R1*/
   {
	...
	/*Blue modify for Inno SDIO card*/
	if((p->block) && (MSDC_Reg32(SDC_STA) & SDC_STA_DATBUSY) )
	  SD_StopTrans_poll();
   SDIO_stop();	
	return NO_ERROR;
	ERROR_EXIT:
