...
#if defined(IC_MODULE_TEST) && defined(MT6260) 
//Not build debug code in MT6260 SLT to reduce Single Banck Code size
#else
...
#endif //defined(IC_MODULE_TEST) && defined(MT6260)
#endif
...
#if defined(MT6260)
#define SF_PROG_TIME_LIMIT 1500  // Program Time Limit: 1.5 ms, because MT6260 use 128byte page program
#else
...
int SF_DAL_OTPWrite_WINBOND(void *MTDData, kal_uint32 Offset, void * BufferPtr, kal_uint32 Length)
{
    ...
    for (i=0; i<Length; i++, bptr++, optr_logical++)    {
        ...
        if(D->FlashID[1]!=0x40) //Only switch to SPI for QPI device
        {
        ...
        if(D->FlashID[1]!=0x40) //Only switch to SPI for QPI device, Quad bit is default set to 1 and Wrap setting is not changed by 0xFF command for QPI device
        {
...
/*!
  @brief
    ...
    Currently, only for MT6250 MT6260.
