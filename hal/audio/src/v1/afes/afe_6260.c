...
static void _AFE_Switch_IntAmp(kal_bool on)  // 1: turn on, 0: turn off
{
   ...
   if(on){   	  
      ...
      *ABBA_VOICE_CON0 &= (~RG_VBIRX_ZCD_EN);
      restore =  *ABBA_VOICE_CON0;  
      *ABBA_VOICE_CON0 = (restore & ~0x00F0) | 0x00B0;  // Default 0dB => [7:4] = 0xB
      {                                           
      ...
      *SPK_CON7 |= 0x04A0; 
      {
      	 kal_int16 targ_gain = restore & 0x00F0;
      	 kal_int16 curr_gain = *ABBA_VOICE_CON0 & 0x00F0;
      	 kal_int16 gain_step = targ_gain >= curr_gain ? 0x0010 : -0x0010;
         while (curr_gain != targ_gain)
         {
            curr_gain += gain_step;
            *ABBA_VOICE_CON0 = (kal_uint16)curr_gain | (restore & 0xFF0F);
            AFE_DELAY(500);
         }
      }
      *ABBA_VOICE_CON0 |= (RG_VBIRX_ZCD_EN);
...
static void AVolumeRampDown() 
{
    ...
    // Voice buffer ramp to 0dB ([7:4] = 0xB)
    {
        *ABBA_VOICE_CON0 &= (~RG_VBIRX_ZCD_EN);
        
        if (!afe.loopback)
        {
            kal_uint16 restore = *ABBA_VOICE_CON0;
            kal_int16 targ_gain = 0x00B0;
            kal_int16 curr_gain = *ABBA_VOICE_CON0 & 0x00F0;
            kal_int16 gain_step = targ_gain >= curr_gain ? 0x0010 : -0x0010;
            while (curr_gain != targ_gain)
            {
...
static kal_int32 Convert_DC_offset_sum_to_DC_offset_val(kal_int32 DC_offset_sum, kal_int32 cali_times, kal_int32 pre_offset_val)
{
    ...
    if (cali_times > 0)
    {
        kal_int32 residual = DC_offset_sum >= 0 ? cali_times : -cali_times;
        kal_int32 DC_offset_diff = ((DC_offset_sum << 1) + residual) / (cali_times << 1);
...
// Set the regression line : y = a * x + b
// y : DSP compensation value
// x : DC offset value
#define DC_CALI_REGRESSION_COEF_A_12dB 18
#define DC_CALI_REGRESSION_COEF_B_12dB 1
#define DC_CALI_REGRESSION_COEF_A_6dB  (DC_CALI_REGRESSION_COEF_A_12dB << 1)
#define DC_CALI_REGRESSION_COEF_B_6dB  DC_CALI_REGRESSION_COEF_B_12dB
static kal_uint16 Convert_DC_offset_val_to_DC_comp_val(kal_int32 DC_offset_val)
{
    kal_int32 coef_a;
    kal_int32 coef_b;
    kal_int16 DC_comp_val;
    
    if ((*SPK_CON0 & 0x0030) == 0x30)
    {   // SPK +12dB
        coef_a = DC_CALI_REGRESSION_COEF_A_12dB;
        coef_b = DC_CALI_REGRESSION_COEF_B_12dB;
    }
    else
    {   // SPK +6dB
        coef_a = DC_CALI_REGRESSION_COEF_A_6dB;
        coef_b = DC_CALI_REGRESSION_COEF_B_6dB;
    }
    
    DC_comp_val = (kal_int16)BOUNDED(DC_offset_val * coef_a + coef_b, 32767, -32768);
    
    return (kal_uint16)DC_comp_val;
}

// for debug
// AuxADC digital value resolution: 10 bits --> 0 ~ 1023
// AuxADC digital value 1024 map to 2800 mV
// DC offset mV = DC offset value * 2800 / 1024
//              = DC offset value *  175 /   64
//              = DC offset value *  175 >>   6
static kal_int16 Convert_DC_offset_val_to_DC_offset_mV(kal_int32 DC_offset_val)
{
    kal_int16 DC_offset_mV = (kal_int16)(DC_offset_val * 175 >> 6);
    return DC_offset_mV;
}

static kal_int32 Average_AuxADC_Value(kal_uint16 *p_buf, kal_int32 count)
{
    kal_int32 avg_value = 0;
    ...
    if (count > 0)
    {
        ...
        avg_value = BOUNDED(total_sum / count, DC_CALI_AUXADC_MAX, DC_CALI_AUXADC_MIN);
    }
...
void DC_Calibration(void)
{
    ...
    {
        // Turn on die2die interface and clk
        _AFE_MDIE_CtrlSeq(KAL_TRUE, KAL_TRUE, AFE_DIE2DIE_DOWNLINK);
        ...
        *AFE_DAC_TEST  |= 0x6000;   // Audio sine tone gen
        ...
        // Turn on audio DAC & buffer
        AFE_Switch_VoiceDAC_AudAMP(0,0);
        
        *ABBA_VOICE_CON0 = (*ABBA_VOICE_CON0 & ~0x00F0) | 0x00B0;  // Default 0dB => [7:4] = 0xB
        _AFE_Switch_IntAmp(KAL_TRUE);
        *SPK_CON0 = ( (*SPK_CON0&0xFFCF)|(SpkAmpGain<<4) );
        AFE_DELAY(1000);
    ...
    
    {
        ...
        for (L = 0; L < DC_CALI_TIMES; L++)
        {
            ...
            for (K = 0; K < 2; K++)
            {
                ...
                *SPK_CON8 |= 0x0040;
                ...
                avg_AuxADC_val[K] = Average_AuxADC_Value(aux_adc_buf, DC_CALI_BUF_LEN);
                afe_prompt_trace(MOD_L1SP, "(# %d) Average AuxADC value [%d] = %d ", L, K, avg_AuxADC_val[K]);
            ...
            DC_offset_sum   += avg_AuxADC_val[0] - avg_AuxADC_val[1];
            DC_offset_val[L] = avg_AuxADC_val[0] - avg_AuxADC_val[1]; // for debug
        ...
        // Convert DC offset value to mV & compensation value
        {
            ...
            afe.DC_offset_val  = Convert_DC_offset_sum_to_DC_offset_val(DC_offset_sum, DC_CALI_TIMES, afe.Pre_offset_val);
