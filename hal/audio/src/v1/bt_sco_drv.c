...
static void btsco_ReadFromBT(BT_SCO_PACKET_LEN uLen, kal_uint32 uPacketLength, kal_uint32 uPacketNumber, kal_uint32 uBlockSize, kal_uint32 uControl)
{
   ...
   for(i=0;i<uBlockSize;i++)
   {
      ...
      btsco.pRX->PacketValid[btsco.pRX->iPacket_w & SCO_RX_PACKET_MASK] = uControl & btsco_PacketValidMask[uLen][i] ;
      btsco.pRX->iPacket_w++;
   }
         ...
         if(btsco.pRX->iPacket_w != btsco.pRX->iPacket_r)
         {
            ...
            {  //do PLC
               ...
               {  //packet lost
                  ...
                  kal_prompt_trace(MOD_L1SP, "[SCO][DRV]PLC Runing at this frame: %d", btsco.pRX->iPacket_r);
               }
            }
   ...
   if( CVSD_LOOPBACK_DATA_BUFFER_SIZE - (cvsd_record.uCurrentWrite - cvsd_record.uCurrentRead) >= uDataLength )
   {
      ...
      for(i=0;i<uPacketNumber;i++)
      {
         ...
         if( ((uControl>>i)&1) == 0)
         {
            ...
            {
               kal_prompt_trace(MOD_L1SP,"[SCO][LB]CVSD_Loopback_hisr, CRC ERROR: uLostPacket: %d, uTotalPacket: %d", cvsd_record.uLostPacket, cvsd_record.uTotalPacket);
