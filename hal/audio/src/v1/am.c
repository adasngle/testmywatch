...
#if defined(MT6235) || defined(MT6235B) || defined(MT6268) || defined(MT6253) || defined(MT6236) || defined(MT6236B)|| defined(MT6256_S00) || defined(MT6256_S01) || defined(MT6255)|| defined(MT6251) || defined(MT6253E) || defined(MT6253L)|| defined(MT6252) || defined(MT6252H) || defined(MT6250) || defined(MT6260)
...
#if defined(MT6256_S00) || defined(MT6256_S01) || defined(MT6255) || defined(MT6250) || defined(MT6251) || defined(MT6260)
...
#if defined(__VIBRATION_SPEAKER_SUPPORT__) && defined(MT6260)
...
#elif defined(MT6268) || defined(MT6236) || defined(MT6236B) || defined(MT6256_S00) || defined(MT6256_S01) || defined(MT6255) || defined(MT6250) || defined(MT6260)
      if(speech_mode>=0x20 && speech_mode<=0x28) { /* AMR-WB */
         *COMPEN_BLOCK_FLT_PAR_PTR &= ~( VOICE_INPUT_FILTER_FLAG + VOICE_OUTPUT_FILTER_FLAG );
         *DP_AUDIO_PAR |= (VOICE_WB_AMR_FLAG);
                                           /* Set AMR-WB bit */
...
static void AM_DSP_Melody( uint32 data )
{
   if( (bool)data ) {            /* Turn on */
        ASSERT_REBOOT(!am.afe_loopback_flag);   // Should not use AFE loopback and melody at the same time
      ASSERT_REBOOT( !(am.state & AM_CONFLICT_STATE) );
...
{
   AM_Enqueue( AM_DSP_Melody, true, false );
   AFE_TurnOnSpeaker( L1SP_VOICE );
}
...
{
   AFE_TurnOffSpeaker( L1SP_VOICE );
   AM_Enqueue( AM_DSP_Melody, false, false );
}
...
#endif
   *AFE_VMCU_CON1 &= ~0x0200;
#elif  defined(MT6268) || defined(MT6236) || defined(MT6236) || defined(MT6256_S00) || defined(MT6256_S01) || defined(MT6255) || defined(MT6250) || defined(MT6260)
   *COMPEN_BLOCK_FLT_PAR_PTR &= ~( VOICE_INPUT_FILTER_FLAG + VOICE_OUTPUT_FILTER_FLAG );
   *DP_AUDIO_PAR |= (VOICE_WB_AMR_FLAG);
   *DP_AUDIO_PAR &= ~( VOICE_SIDETONE_FILTER_FLAG );
   *DP_16k_CompenFlt_Flag = 1;
...
void AM_AudioPlaybackOff( bool wait )
{
   ...
   AFE_TurnOffSpeaker( L1SP_TONE );
   AFE_TurnOffSpeaker( L1SP_KEYTONE );
#if __KARAOKE_SUPPORT__ // to avoid pop noise
   if(AFE_IsKaraok())
   	 kal_sleep_task(3);  
#endif 
        ...
        case AM_PCMEX_TYPE_IDLE:        // 2:   // PCM2WAY VoIP
            {
                ...
                {
                    ASSERT_REBOOT(!am.afe_loopback_flag && !(am.state & AM_CONFLICT_STATE));
                    idle = am.state == 0 ? true : false;
...
static uint16 Pseudo_SALI_PcmEx_GetBufLen(Pseudo_Sal_PcmEx_BufId_t id)
{
	...
	switch(id)
	{
		...
		case PSEUDO_SAL_PCMEX_PNW_BUF_DL2:
			...
			break;
		case PSEUDO_SAL_PCMEX_BUF_SE:
			len = *DSP_SPH_BAND & 0x2 ? 320 : 160;  //bit1 -> codec band, bit0 -> sch band
			break;
		case PSEUDO_SAL_PCMEX_BUF_SD:
			len = *DSP_SPH_BAND & 0x1 ? 320 : 160;  //bit1 -> codec band, bit0 -> sch band
			break;
		case PSEUDO_SAL_PCMEX_DACA_BUF_UL:
        ...
        case PSEUDO_SAL_PCMEX_BUF_IDLE_SE:
            len = *DSP_SPH_BAND_FLAG & 0x1 ? 320 : 160;  //bit1 -> DL, bit0 -> UL
            break;
