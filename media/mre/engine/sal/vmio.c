...
#define NFB_HDL_ID_BASE 32768
extern WCHAR *mmi_wcsstr(WCHAR *string, const WCHAR *str_char_set);
extern VMINT8 vm_get_exec_cid(VMWSTR wfilename);
extern VMINT vm_is_nfb_file(VMWSTR filename);
static VMINT nfb_seek_size = 0;
static VMINT _vm_file_is_3rd_rom(const U16 * filename)
{
    if ((mmi_wcsstr((WCHAR *)filename, (const WCHAR *)L".nfb") || mmi_wcsstr((WCHAR *)filename, (const WCHAR *)L".NFB")) && 
        L'@' == filename[0])
    {
        return 1;
    }
...
static VMFILE _vm_file_open_internal(const VMWSTR filename, VMUINT mode, VMUINT binary, VMINT is_remap)
{
    ...
    VMWCHAR temp[MAX_APP_NAME_LEN] = {0};
    VMINT is_3rd_rom = vm_is_nfb_file((VMWSTR)filename); //_vm_file_is_3rd_rom((const U16 *)filename);
    if (is_3rd_rom)
    {
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
    if (handle >= NFB_HDL_ID_BASE)
    {
        is_nfb = 1;
        handle -= NFB_HDL_ID_BASE;
    }
#endif
    ...
    if (vm_res_get_mtk_reshandle(VM_RES_TYPE_FILE, handle, &mtk_file_id) == VM_RES_OK)
        {
            ...
            {
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
                if (NFB_RES_Load(mtk_file_id, data, nfb_seek_size, length))
                {
                    *nread =  length;
                    nfb_seek_size += length;
                }
                else
                {
                    *nread = 0;
            ...
            else if (FS_Read(mtk_file_id, data, length, (UINT *)nread) < FS_NO_ERROR)
			{
				*nread = 0;
			}
		}
...
VMINT vm_file_seek(VMFILE handle, VMINT offset, VMINT base) 
{
    ...
    if (NFB_HDL_ID_BASE <= handle)
    {
        ...
        case BASE_CURR:
            break;
        case BASE_END:
            if (vm_res_get_mtk_reshandle(VM_RES_TYPE_FILE, handle - NFB_HDL_ID_BASE, &mtk_file_id) == VM_RES_OK)
                nfb_seek_size = NFB_RES_get_content_size(mtk_file_id);
            break;
        default :
            nfb_seek_size = 0;;
            break;
        }
        nfb_seek_size += offset;
        return 0;
    }
...
void vm_cui_popup(U16 id)
{
        ...
        mmi_frm_nmgr_popup(MMI_SCENARIO_ID_DEFAULT,MMI_EVENT_WARNING,(WCHAR*)GetString(id));  

#endif    
}

MMI_BOOL vm_app_config_mem_check(mmi_id appId)
{
    U32 size = mmi_res_get_app_fg_mem_size(appId) + mmi_res_get_app_base_mem_size(appId);
    return mmi_frm_asm_check_enough(appId, size);
}
