...
VMINT vm_start_mre_app(VMWSTR fileName, vm_mre_app_evt_cb_t vam_cb)
{
    ...
    preload_info.heapSize -= sizeof(vm_preload_app_info_t);
    total_mem = preload_info.loadSize+preload_info.heapSize+sizeof(vm_preload_app_info_t);
    MMI_PRINT(MOD_MRE,TRACE_GROUP_8,"[MRE]start mem %d, %d",total_mem,sysconf.memory_size);
    if (total_mem > sysconf.memory_size)
    {
        error_pos = 3;
        ret = VM_PMNG_NO_MEM;
        goto error;
...
static void vm_update_callback(VMINT hdl, void * para)
{
    ...
    if (E_PRELOAD_AVAILABLE_UPDATE == data.head.state ||
        ...
        E_PRELOAD_PIPE_CLOSED == data.head.state ||
        E_PRELOAD_PIPE_BROKEN == data.head.state ||  
        E_PRELOAD_ERR_PATH == data.head.state|| 
        E_PRELOAD_ABORT == data.head.state)
    {        
...
VMINT vm_update_check_launch(VMWSTR path, VMWSTR new_path)
{
    ...
    length = kal_wstrlen(new_path)-3;

    while (suffix[i][0])
    {
        kal_wstrcat(new_path,(const WCHAR *)suffix[i]);
