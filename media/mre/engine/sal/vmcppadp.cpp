...
extern "C" MMI_BOOL vm_app_config_mem_check(mmi_id appId);
extern "C" void vm_cui_popup(VMUINT32 id);
...
extern "C" MMI_ID vm_vapp_dtcnt_select_run(VM_P_HANDLE p_hdl, VMINT (*cb)(VMUINT32 dtacct), VMINT sim_id)
{
    ...
    VMINT check = vm_app_config_mem_check(VAPP_MRE_APP_DTCNT);
    if (!check)
    {
        vm_cui_popup(STR_GLOBAL_NOT_ENOUGH_MEMORY);
