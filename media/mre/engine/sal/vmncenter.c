...
extern void vm_pmng_set_from_ncenter_flag();
typedef struct
{
    U16* path;
    U8*  img;
...
static VMWCHAR g_path[260];

static MMI_RET vm_ncenter_ongoing_launch_cb(mmi_event_struct *evt)
{
        vm_pmng_set_from_ncenter_flag();
    if (0 != srv_mre_launch_app_ex(g_path,NULL))
    {
...
void vm_ncenter_ongoing_cb(vsrv_notification_handle handle, vsrv_nintent intent, void* userData, U32 userDataSize)
{
    ...
    if (VSRV_NINTENT_TYPE_LAUNCH == intent.type)
    {
	    ...
	    MMI_FRM_POST_EVENT(&evt, vm_ncenter_ongoing_launch_cb, NULL);
