...
VMINT vm_midi_play_ex(VMINT resid, VMUINT start_time, VMINT repeat, VMUINT path, void (*f)(VMINT handle, VMINT event))
{
	...
	if (vm_get_res_provider())
	{
		rawdata = (vm_get_res_provider())(resid, &len);
		if (!rawdata)
		{
		...
		{
			return vm_midi_play_by_bytes_ex( rawdata, len, start_time, repeat, path, f);
		}
	}
	return VM_MIDI_FAILED;
}
...
{
	return vm_midi_play_ex(resid,0,repeat,VM_DEVICE_SPEAKER2,f);
}
