...
VMINT get_nfb_res_offset(VMINT cid, VMINT nfb_offset, char *res_name, VMINT *res_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	char tmp[128], * org_name = NULL;
	VMINT len = 0, offset = -1;
	VMUINT nread;
	VMINT offset_on = nfb_offset;
	VMINT total_size;
	...
	total_size = NFB_RES_get_content_size(cid);
	...
	while (TRUE) 
	{
		len = 0;
		do {
			if(NFB_RES_Load(cid,tmp + len, offset_on, 1) != 0)
			{
				offset_on++;
				if (tmp[len] == '\0')
					break;
		...
		{
			if(NFB_RES_Load(cid,&offset, offset_on, 4) == 0)
			{
				offset = -1;
			}
			else if(res_size != NULL && NFB_RES_Load(cid, res_size, offset_on + 4, 4) == 0)
			{
				offset = -1;
			}
			
			offset_on = offset_on + 8;
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)	
	else if(vm_parse_exec_format(filename) == VM_EXEC_NFB)
	{
		VMINT nfb_offset;
		VMINT cid = vm_get_exec_cid(filename);
		
		nfb_offset = open_nfb_file_for_load(filename, (VMUINT*)&res_offset);
		...
		if ((offset_temp = get_nfb_res_offset(cid, nfb_offset, res_name, res_size)) > -1) 
		{	
			*offset = offset_temp + res_offset;
			...
			if(is_load == TRUE)
			{
				if(buffer == NULL)
				{
					if ((res = vm_malloc(*res_size)) != NULL) 
					{
						if(NFB_RES_Load(cid,(VMCHAR *)res, offset_temp + res_offset, *res_size) == 0)
						{
							vm_free(res);
							res = NULL;
						}
			...
			{
				if(NFB_RES_Load(cid,(VMCHAR *)buffer, offset_temp + res_offset, *res_size) == 0)
				{
					res = NULL;
				}
			}
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)	
	else if(vm_parse_exec_format(exec_filename) == VM_EXEC_NFB)
	{
		VMINT nfb_offset;
		VMINT cid = vm_get_exec_cid(exec_filename);
		
		nfb_offset = open_nfb_file_for_load(exec_filename, (VMUINT*)&res_offset);
		...
		if ((offset_temp = get_nfb_res_offset(cid, nfb_offset, res_name, res_size)) > -1) 
		{	
			offset_temp = offset_temp + res_offset;
			
			if ((res = vm_malloc(*res_size)) != NULL) 
			{
				if(NFB_RES_Load(cid,(VMCHAR *)res, offset_temp, *res_size) == 0)
				{
					vm_free(res);
					res = NULL;
				}
...
VMBOOL vm_load_resource_use_outside_memory(VMWSTR filename, char *res_name, void* buffer, VMINT buffer_size, vm_res_read_hint_struct* hint)
{
	...
	if( buffer == NULL )
    {
    	buff = _vm_load_resource(filename, res_name, &res_size, NULL, &offset, FALSE);
		{
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)	
VMINT open_nfb_file_for_load(VMWSTR filename, VMUINT* res_offset) 
{
	vm_elf_handle_t elf_hdl;
	vm_file_type_enum file_type;
	VMINT8 cid;
	...
	if(( cid = vm_get_exec_cid(filename)) == 0)
		return VM_EX_FMTERR;
	
	file_size = NFB_RES_get_content_size(cid);
	if(file_size == 0)
		return VM_EX_FMTERR;
	...
	if(file_type  == VM_SIMPLE_VXP_ADS)
	{
		...
		if(NFB_RES_Load(cid,(VMCHAR *)&offset, file_size-0x0C, sizeof(VMINT)) == 0)
		{
			return	VM_EX_FMTERR;
		}

		
		if(NFB_RES_Load(cid,(VMCHAR *)&tatal_size, offset - 0x04, sizeof(VMINT)) == 0)
		{
			return	VM_EX_FMTERR;
		}
		...
		if(sizeof(vm_simple_elf_info) == tatal_size)
		{			
			if(NFB_RES_Load(cid, &info, offset - tatal_size -  0x04, tatal_size) == 0)
			{
				return	VM_EX_FMTERR;
			}
	...
	else
	{
		...
		if(open_nfb_elf(filename, &elf_hdl) == 0) 
		{
			if (read_nfb_elf_section_head(&elf_hdl, elf_hdl.elf_head.e_shstrndx, &str_sh) == 0) 
			{
				if ((sh_str_tbl = vm_malloc(str_sh.sh_size)) != NULL) 
				{
					if (read_nfb_elf_section(&elf_hdl, &str_sh, sh_str_tbl) == 0) 
					{
						if (read_nfb_elf_section_head_by_name(&elf_hdl, ".vm_res", sh_str_tbl,
								str_sh.sh_size, &res_sh) == 0)
						{
							if(rorw_zip)
								*res_offset = res_sh.sh_offset;
...
VMUINT vm_get_resource_offset_from_file(VMWSTR filename, char *res_name)
{
	...
	if(vm_is_nfb_file(filename))
	{
		VMWCHAR temp[MAX_APP_NAME_LEN];
		VMINT length = vm_wstrlen(filename);
		vm_wstrcpy(temp, filename);
		temp[0] = '@';
		temp[length-1] = 'b';
		temp[length-2] = 'f';
		temp[length-3] = 'n';
		return _vm_get_resource_offset(temp, res_name, NULL);
	}
...
VMUINT vm_get_resource_offset_and_size_from_file(VMWSTR filename, char *res_name, VMINT* res_size)
{
	if(vm_is_nfb_file(filename))
	{
		...
		temp[length-2] = 'f';
		temp[length-3] = 'n';
		return _vm_get_resource_offset(temp, res_name, res_size);
	}
...
VMINT32 vm_resource_get_data_from_file(VMWSTR filename, VMUINT8 *data, VMUINT32 offset, VMUINT32 size)
{
	...
	memset(temp,0,MAX_APP_NAME_LEN);
    ...
    exec_format = vm_parse_exec_format(temp);
	...
	if (exec_format == VM_EXEC_ROM)
	{
		...
		file_type = vm_get_file_type(temp);
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)	
	//else if(vm_is_nfb_file(filename))
	else if(exec_format == VM_EXEC_NFB)
	{
		VMINT nfb_offset;
		VMINT cid;
		
		cid = vm_get_exec_cid(temp);
		
		nfb_offset = open_nfb_file_for_load(temp, (VMUINT*)&res_offset);
		...
		if ((VMINT32)offset  >= 0) 
        {
        	if(NFB_RES_Load(cid,data, offset + res_offset, size) == 0)
			{
					data = NULL;
			}
		}
	...
	{
		file = open_elf_file_for_load(temp, (VMUINT*)&res_offset);
...
VMINT32 vm_res_init_with_language(VMUINT32 language)
{
    ...
    {		
		...
		if(format != VM_EXEC_ROM && format != VM_EXEC_NFB)
		{
			if(pcb->fileHandle < 0)
			{
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)	
		else if(vm_parse_exec_format(pcb->fullFileName) == VM_EXEC_NFB)
		{
			VMINT nfb_offset;
			VMINT cid = vm_get_exec_cid(pcb->fullFileName);
			
			nfb_offset = open_nfb_file_for_load(pcb->fullFileName, (VMUINT*)&res_offset);
			...
			if(NFB_RES_Load(cid,res_data, pcb->res_offset + res_offset, res_size) == 0)
			{
				return NULL;
			}
		}
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)	
		else if(vm_parse_exec_format(sm_ptr->fullFileName) == VM_EXEC_NFB)
		{
			VMINT nfb_offset;
			VMINT cid = vm_get_exec_cid(sm_ptr->fullFileName);
			
			nfb_offset = open_nfb_file_for_load(sm_ptr->fullFileName, (VMUINT*)&res_offset);
...
VMUINT8 *vm_res_get_virtual_image(VMUINT32 image_id)
{
	...
	else if(vm_parse_exec_format(pcb->fullFileName) == VM_EXEC_NFB)
	{
		...
		if ((res_data = vm_malloc(res_size)) != NULL) 
		{
			if(NFB_RES_Load(cid,(VMCHAR *)res_data, pcb->res_offset + res_offset, res_size) == 0)
			{
				vm_free(res_data);
				return NULL;
			}
