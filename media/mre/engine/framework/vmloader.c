...
static VMINT load_rom_zipped_ads(vm_load_context_t *context)
{
	...
	if (vm_get_vm_tag(context->fileName, VM_CE_INFO_MEM_REQ, &memSize, &bufSize) 
   ...
   memSize = (memSize * 1024);
    ...
    if (context->preload_flags != VM_LOAD_PRELOAD_READ) {
        ...
        memSize = memSize - sizeof(vm_preload_app_info_t) - elf_info.org_rw_size;
		
        if ((elf_info.org_ro_size + elf_info.org_rw_size + VM_UNZIP_MEM_SIZE) <= memSize ) //context->load_size) 
		{
            runtime_in_app_mem = 1;
            runtime_base = context->load_buffer + memSize /*context->load_size*/ - VM_UNZIP_MEM_SIZE;
        }
        else {
...
static VMINT load_file_zipped_ads(vm_load_context_t *context)
{
   ...
   memSize = memSize * 1024;
    ...
    if (context->preload_flags != VM_LOAD_PRELOAD_READ) {
        ...
        if ((elf_info.org_ro_size + elf_info.org_rw_size + VM_UNZIP_MEM_SIZE + rbuf_size) <= memSize) //context->load_size )
        {
            /* both uncompress runtime memory and read file buffer can be in app memory */
            runtime_in_app_mem = 1;
            rbuf_in_app_mem = 1;
        }
        else if ((elf_info.org_ro_size + elf_info.org_rw_size + max2(VM_UNZIP_MEM_SIZE, rbuf_size)) <= memSize) //context->load_size ) 
        {
        ...
        else if ((elf_info.org_ro_size + elf_info.org_rw_size + min2(VM_UNZIP_MEM_SIZE, rbuf_size)) <= memSize) //context->load_size )
        {
		...
		mem_tail = context->load_buffer + memSize;
...
static VMINT load_nfb_normal_ads(vm_load_context_t *context)
{
    ...
    ret = open_nfb_elf(context->fileName, &handle);
    if (ret != 0) {
        lerr = 1; goto open_failed;
    }

    ret = read_nfb_elf_prog_head(&handle, &ph);
    if (ret != 0) {
        lerr = 2; goto read_hdr_failed;
    }
    ret = read_nfb_elf_section_head(&handle, handle.elf_head.e_shstrndx, &str_sh);
    if (ret != 0) {
        lerr = 3; info = handle.elf_head.e_shstrndx; goto read_hdr_failed;
    }
    str_tab = _vm_kernel_malloc(str_sh.sh_size);
    ...
    ret = read_nfb_elf_section(&handle, &str_sh, str_tab);
    if (ret != 0) {
        lerr = 5; goto free_strtab;
    }
    ret = read_nfb_elf_section_head_by_name(&handle, "ER_RO", str_tab, str_sh.sh_size, &ro_sh);
    if (ret != 0) {
        lerr = 6; goto free_strtab;
    }
    ret = read_nfb_elf_section_head_by_name(&handle, "ER_RW", str_tab, str_sh.sh_size, &rw_sh);
    if (ret != 0) {
        lerr = 7; goto free_strtab;
    }
    if (context->preload_flags != VM_LOAD_PRELOAD_READ) {
        ...
        ret = read_nfb_elf_section(&handle, &ro_sh, context->load_buffer);
        if (ret != 0) {
            lerr = 9; goto free_strtab;
        }
        ret = read_nfb_elf_section(&handle, &rw_sh, context->load_buffer + ro_sh.sh_size);
        if (ret != 0) {
            lerr = 10; goto free_strtab;
        }
    }
...
static VMINT load_nfb_zipped_ads(vm_load_context_t *context)
{
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
    vm_simple_elf_info elf_info;
    VMFILE handle;
   ...
   memSize = memSize * 1024;
    vm_get_section_info_from_nfb(context->fileName, &elf_info);
    ...
    if (context->preload_flags != VM_LOAD_PRELOAD_READ) {
		...
		cid = vm_get_exec_cid(context->fileName);

		ret = NFB_RES_Load(cid, rbuf,  elf_info.ro_offset , elf_info.ro_size);
		...
		ret = NFB_RES_Load(cid, rbuf,  elf_info.rw_offset , elf_info.rw_size);
...
static VMINT do_sysv_nfb_loadfix(vm_load_context_t *context, vm_elf_head_t *elf_head, 
                                    char *str_tab, VMUINT str_size,
                                    vm_elf_section_head_t *sh_tab, VMINT sh_num)
{
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
    ...
    ro_sh = get_nfb_elf_section_head_by_name(".text", str_tab, str_size, sh_tab, sh_num);
    if (ro_sh == NULL) {
        lerr = 1; goto bad;
    }
    rw_sh = get_nfb_elf_section_head_by_name(".data", str_tab, str_size, sh_tab, sh_num);
    if (rw_sh == NULL) {
        lerr = 2; goto bad;
    }
    ...
    sh = get_nfb_elf_section_head_by_name(".dynstr", str_tab, str_size, sh_tab, sh_num);
    if (sh != NULL) {
        if (!(sh->sh_flags & VM_SHF_ALLOC)) {
            lerr = 3; goto bad;
        }
    ...
    sh = get_nfb_elf_section_head_by_name(".dynsym", str_tab, str_size, sh_tab, sh_num);
    if (sh != NULL) {
        if (!(sh->sh_flags & VM_SHF_ALLOC)) {
            lerr = 4; goto bad;
        }
    ...
    sh = get_nfb_elf_section_head_by_name(".init_array", str_tab, str_size, sh_tab, sh_num);
    if (sh != NULL) {
        if (!(sh->sh_flags & VM_SHF_ALLOC)) {
            lerr = 5; goto bad;
        }
    ...
    sh = get_nfb_elf_section_head_by_name(".rel.dyn", str_tab, str_size, sh_tab, sh_num);
    if (sh != NULL) {
        if (!(sh->sh_flags & VM_SHF_ALLOC)) {
            lerr = 6; goto bad;
        }
...
/* under construction !*/
    ...
    sh = get_nfb_elf_section_head_by_name(".rel.plt", str_tab, str_size, sh_tab, sh_num);
    if (sh != NULL) {
        if (!(sh->sh_flags & VM_SHF_ALLOC)) {
            lerr = 7; goto bad;
        }
...
static VMINT do_nfb_sysv_loadfix(vm_load_context_t *context, vm_elf_handle_t *handle)
{
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
	vm_elf_section_head_t *sh_tab, *str_sh;
    VMINT sh_tabsize;
	...
	ret = NFB_RES_Load(handle->file, sh_tab, handle->elf_head.e_shoff, sh_tabsize);
...
static VMINT load_rom_zipped_sysv(vm_load_context_t *context)
{
   ...
   memSize = ((memSize-10) * 1024);
    ...
    if (context->preload_flags != VM_LOAD_PRELOAD_READ) {
        ...
        if ((com_info.pt_load_new_offset + com_info.pt_load_old_filesize + VM_UNZIP_MEM_SIZE) <= memSize ) //context->load_size) 
		{
            runtime_in_app_mem = 1;
            runtime_base = context->load_buffer + memSize /* context->load_size */ - VM_UNZIP_MEM_SIZE;
        }
...
static VMINT load_file_zipped_sysv(vm_load_context_t *context)
{
   ...
   memSize = ((memSize-10) * 1024);
        ...
        if ((com_info.pt_load_new_offset + com_info.pt_load_old_filesize + VM_UNZIP_MEM_SIZE + rbuf_size) <= memSize) // context->load_size)
        {
            runtime_in_app_mem = 1;
            rbuf_in_app_mem = 1;
        }
        else if((com_info.pt_load_new_offset + com_info.pt_load_old_filesize + max2(VM_UNZIP_MEM_SIZE, rbuf_size)) <= memSize) //context->load_size)
        {
            if (max2(VM_UNZIP_MEM_SIZE, rbuf_size) == VM_UNZIP_MEM_SIZE) {
                runtime_in_app_mem = 1;
                rbuf_in_app_mem = 0;
        ...
        else if((com_info.pt_load_new_offset + com_info.pt_load_old_filesize + min2(VM_UNZIP_MEM_SIZE, rbuf_size)) <= memSize ) //context->load_size)
        {
            if (min2(VM_UNZIP_MEM_SIZE, rbuf_size) == VM_UNZIP_MEM_SIZE) {
                runtime_in_app_mem = 1;
                rbuf_in_app_mem = 0;
...
static VMINT load_nfb_zipped_sysv(vm_load_context_t *context)
{
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
	vm_elf_handle_t handle;
    vm_elf_prog_head_t ph;
    ...
    load_count = 0;
    for (index = 0; index < handle.elf_head.e_phnum; index++) {
        ret = read_nfb_elf_prog_head_by_index(&handle, index, &ph);
        if (ret != 0) {
            lerr = 2; info = index; goto read_failed;
        }
   ...
   memSize = ((memSize-10) * 1024);
        ...
        vm_get_compress_info_from_nfb(context->fileName, &com_info);
        rbuf_size = com_info.pt_load_new_filesize;
        ...
        /* read the uncompress part in head of file, include ELF header & table */
		ret = NFB_RES_Load(handle.file, context->load_buffer,  0 , com_info.pt_load_new_offset);

        if (ret <= 0) {
            lerr = 9; info = nread; goto free_unzip_adm;
        }
    
		ret = NFB_RES_Load(handle.file, rbuf, com_info.pt_load_new_offset, com_info.pt_load_new_filesize);
...
static VMINT load_nfb_normal_sysv(vm_load_context_t *context)
{
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
	vm_elf_handle_t handle;
    vm_elf_prog_head_t ph;
    ...
    if (context->preload_flags != VM_LOAD_PRELOAD_READ) {
        ...
        for (index = 0; index < handle.elf_head.e_phnum; index++) {
			...
			ret = NFB_RES_Load(handle.file, context->load_buffer + (ph.p_vaddr - base_virt), ph.p_offset, ph.p_filesz);
...
static const vm_load_file_t load_file_functions[3][2][2] = 
{
    /* ROM */
    {
        /* zipped */
	...
	{
		/* zipped */
		{ /* ADS */ load_nfb_zipped_ads, /* RVCT&GCC */ load_nfb_zipped_sysv },
		/* normal */
		{ /* ADS */ load_nfb_normal_ads, /* RVCT&GCC */ load_nfb_normal_sysv }
	}
};
...
VMINT vm_load_normal(vm_load_context_t *p_context)
{
	...
	if(p_context->suffix == VM_EXEC_ROM)
		type = 0;
	else if(p_context->suffix == VM_EXEC_NFB)
		type = 2;
	
    load_func = load_file_functions[type][p_context->rorw_zip ? 0 : 1][is_ads ? 0 : 1];
#endif
    /* if (load_func == NULL) ... */
...
VMINT vm_read_preload_app_info(vm_preload_app_info_t *preload_info, const VMWSTR fullFileName)
{
    ...
    /* error checking */
    if (suffix != VM_EXEC_VXP && suffix != VM_EXEC_DLL && suffix != VM_EXEC_ROM && suffix != VM_EXEC_NFB) {
...
VMINT vm_preload_app(vm_preload_app_info_t *preload_info, VMINT flags)
{
    ...
    if (preload_info->saved_context.suffix != VM_EXEC_VXP 
        && preload_info->saved_context.suffix != VM_EXEC_DLL  && preload_info->saved_context.suffix != VM_EXEC_ROM && preload_info->saved_context.suffix != VM_EXEC_NFB) {
