...
int open_elf(VMWSTR filename, vm_elf_handle_t *handle)
{
    ...
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_ELFLDR, TRC_MRE_ELFLDR_372 ,handle->file, sizeof(vm_elf_head_t), nread);
...
int open_nfb_elf(VMWSTR filename, vm_elf_handle_t *handle)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	handle->file = vm_get_exec_cid(filename);
	
	
    if (NFB_RES_Load(handle->file, &handle->elf_head, 0, sizeof(vm_elf_head_t)) == 0 ) 
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_ELFLDR, TRC_MRE_ELFLDR_372 ,handle->file, sizeof(vm_elf_head_t), 0);
        return VM_EX_FMTERR;
    }
...
int read_nfb_elf_prog_head(vm_elf_handle_t *handle, vm_elf_prog_head_t *head)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    for ( phnum = 0; phnum < handle->elf_head.e_phnum; phnum++ )
    {            
        if (NFB_RES_Load(handle->file, head, handle->elf_head.e_phoff + phnum * sizeof( vm_elf_prog_head_t ), sizeof(vm_elf_prog_head_t)) == 0)
        {
                return VM_EX_IOERR;
        }
        else
 ...
 *  
 *****************************************************************************/
int read_nfb_elf_prog_head_by_index(vm_elf_handle_t *handle, int idx, vm_elf_prog_head_t *head)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if ( NFB_RES_Load(handle->file, head, handle->elf_head.e_phoff + idx * sizeof( vm_elf_prog_head_t ), sizeof(vm_elf_head_t)) == 0 )
    {
            return VM_EX_IOERR;
    }
 ...
 *  
 *****************************************************************************/
int read_nfb_elf_section_head(vm_elf_handle_t *handle, int idx, vm_elf_section_head_t *head)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (idx < handle->elf_head.e_shnum) 
	{
        if (NFB_RES_Load(handle->file, head, handle->elf_head.e_shoff + idx * handle->elf_head.e_shentsize, sizeof(vm_elf_section_head_t)) == 0)
            return VM_EX_IOERR;
    }
    else 
	{
 ...
 *  
 *****************************************************************************/
vm_elf_section_head_t *get_nfb_elf_section_head_by_name(
                        char *name,
                        char *str_tab,
                        int tab_size,
                        vm_elf_section_head_t *sh,
 ...
 *  
 *****************************************************************************/
int read_nfb_elf_section_head_by_name(
        vm_elf_handle_t *handle,
        char *name,
        char *string_table,
        int table_size,
        vm_elf_section_head_t *head)
{
    ...
    for (i = 0; i < handle->elf_head.e_shnum; i++) {
        if (read_nfb_elf_section_head(handle, i, head) == 0) {
            if (strcmp(name, get_elf_string(string_table, table_size, 
                head->sh_name)) == 0)
                return 0;
        }
 ...
 *  
 *****************************************************************************/
int read_nfb_elf_section(vm_elf_handle_t *handle, vm_elf_section_head_t *head, void *buf)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (NFB_RES_Load(handle->file, buf,  head->sh_offset, head->sh_size) != 0)
        return 0;
    else
        return VM_EX_IOERR;
}
...
VMINT vm_get_section_info_from_nfb(VMWSTR filename, vm_simple_elf_info *p_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if(file_type  == VM_SIMPLE_VXP_ADS)
    {
        ...
        VMUINT tatal_size;
		VMINT file_size;
		VMINT8 cid = vm_get_exec_cid(filename);

		file_size = NFB_RES_get_content_size(cid);
			
		if (NFB_RES_Load(cid, &offset,  file_size-0x0C , 4) != 0)
		{
            if (NFB_RES_Load(cid, &tatal_size,   offset - 0x04 , 4) != 0)
            {
                if(sizeof(vm_simple_elf_info) == tatal_size)
                {
                	if (NFB_RES_Load(cid, p_info,   offset - tatal_size -  0x04 , tatal_size) != 0)
                	{
                         return 0;
                    }
                }
    ...
    else
    {
        ...
        if (open_nfb_elf(filename, &handle) == 0) 
        {
            if (read_nfb_elf_prog_head(&handle, &ph)== 0) 
            {
                if (read_nfb_elf_section_head(&handle, handle.elf_head.e_shstrndx, &str_sh) == 0)
                {
                    if ((str_tab =  _vm_kernel_malloc(str_sh.sh_size)) != NULL) 
                    {
                        if (read_nfb_elf_section(&handle, &str_sh, str_tab)== 0) 
                        {
                            if (read_nfb_elf_section_head_by_name(&handle, "ER_RW",str_tab, str_sh.sh_size, &rw_sh) == 0)
                            {
                            ...
                            if (read_nfb_elf_section_head_by_name(&handle, "ER_RO",str_tab, str_sh.sh_size, &ro_sh) == 0)
                            {
...
#if defined(__NFB_THIRD_ROM_SUPPORT__) &&  defined(__MTK_TARGET__)
VMINT vm_get_compress_info_from_nfb(VMWSTR filename, vm_compress_elf_info *p_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (file_size > 0) 
    {
        if(NFB_RES_Load(cid, &offset,  file_size-0x0C , 4) != 0)
        {
                if( NFB_RES_Load(cid, p_info,  offset - 0xc , 0xc) != 0)
                {
                    return 0;
                }
        }
