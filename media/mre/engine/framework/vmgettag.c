...
static VMINT8* vm_get_nfb_tag_ptr(VMINT8 cid, VMINT* size)
{
	VMINT		offset = 0;
	VMINT       ret = 0;
	VMUINT		file_size = 0;	
	...
	if(NFB_RES_Load(cid,(VMCHAR *)&offset, file_size-12, sizeof(VMINT)) == 0)
	{
		return	NULL;
	}
	...
	*size = file_size-offset;
	...
	if(*size)
	{
		tag_buffer = (VMCHAR *)_vm_kernel_malloc(*size);
	
		if( tag_buffer != NULL )
		{
			memset( tag_buffer, 0x00, *size);

			if( NFB_RES_Load(cid,tag_buffer, offset, *size) == 0 )
			{
				memset( tag_buffer, 0x00, *size);
			...
			{
				VMUINT8 type = tag_buffer[*size - 8];
				...
				if( memcmp( &tag_buffer[*size - pos], "\xB4VDE10", 6 ) )
				{
					_vm_kernel_free( tag_buffer );
					return NULL;
				}	
	
				memset( &tag_buffer[*size - pos], 0x00, pos );
...
static VMINT vm_get_tag_init_internal(short* filename, VMBOOL internal)
{
	...
	{
		tag_buffer = vm_get_nfb_tag_ptr(cid, &file_size);
	}
	else
#endif
	{
