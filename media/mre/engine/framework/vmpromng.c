...
VMINT vm_get_sym_entry(char* symbol) 
{
	...
	if( returnvalue )
	{
        ...
        returnvalue = vm_get_ssl_sym(symbol);
        if( returnvalue )
        {
            return returnvalue;
        }
#endif
    if(strcmp(symbol,"vm_get_full_path_by_short_name") == 0)
        return (VMINT)vm_get_full_path_by_short_name;
    if(strcmp(symbol,"vm_pmng_delete_process") == 0)
        return (VMINT)vm_pmng_delete_process;        
...
static VMINT g_pmng_ncenter_flag = 0;

VMINT vm_pmng_set_from_ncenter_flag(void)
{
    g_pmng_ncenter_flag = 1;
}

VMINT vm_pmng_get_from_ncenter_flag(void)
{
    VMINT flag = g_pmng_ncenter_flag;
    g_pmng_ncenter_flag = 0;
    return flag;
}
...
VM_P_HANDLE vm_pmng_create_process(VMWSTR fileName, VMUINT8* mem_base)
{
    ...
    /* error checking */
    if (suffix != VM_EXEC_ROM && suffix != VM_EXEC_VXP && suffix != VM_EXEC_DLL && suffix != VM_EXEC_NFB) {
...
VMINT vm_get_exec_filename(VMWSTR filename)
{
	...
	if (filename)
	{
		...
		if ((pcb =  vm_sh_pcb_in_pt(SH_TYPE_R9, currAppR9, &status)) != NULL 
			&& status != VM_PMNG_UNLOAD)
		{
			if(vm_parse_exec_format(pcb->fullFileName) == VM_EXEC_NFB)
			{
				char temp[MAX_APP_NAME_LEN];
				sprintf(temp, "%s%d%s", "T:\\",vm_get_exec_cid(pcb->fullFileName),".vxp");
				vm_ascii_to_ucs2((VMWSTR)filename, MAX_APP_NAME_LEN, (VMSTR)temp);
			}
			else
			{
		...
		else
		{
			...
			if (sm_ptr && sm_ptr->pcb)
			{
				if(vm_parse_exec_format(sm_ptr->pcb->fullFileName) == VM_EXEC_NFB)
				{
					char temp[MAX_APP_NAME_LEN];
					sprintf(temp, "%s%d%s", "T:\\",vm_get_exec_cid(sm_ptr->pcb->fullFileName),".vxp");
					vm_ascii_to_ucs2((VMWSTR)filename, MAX_APP_NAME_LEN, (VMSTR)temp);
				}
				else
				{
...
//	pcb->event = USER_EVENT;
	...
	if (pcb->sysEventHandler)
	{
		...
		_vm_pmng_send_sysevt_msg(pcb, VM_MSG_PAINT, vm_pmng_get_from_ncenter_flag());
	}
...
static VMINT vm_pmng_set_bg_int(_vm_pcb_t* pcb, VM_PROCESS_STATUS status)
{
    ...
    if (pcb->is_support_bg_run)
    {
        ...
        /* send hide message to N-Center */
        if (pcb->is_support_bg_run && kal_wstrcmp(pcb->fullFileName+ kal_wstrlen(pcb->fullFileName)-3,L"vsp"))
        {
...
{
    VMINT pos = 0;
    VMWCHAR file_name[MAX_APP_NAME_LEN+1];
    VMINT length = 0,i = 0;
    VMWSTR p = NULL;

    static const VMWCHAR suffix[][4]=
    {
        {L'v',L'x',L'p',0},
        {L'v',L'p',L'p',0},
        {L'v',L's',L'p',0},
    ...
    vm_get_exec_filename(file_name);
    length = kal_wstrlen(file_name);
    ASSERT(length > 0);
    p = file_name+length-1;
    while ( p != file_name)
    {
        if (L'\\' == *p)
        {
    ...
    kal_wstrcat(file_name,short_name);
    kal_wstrcat(file_name,L".");
    length = kal_wstrlen(file_name);
    ...
    while (suffix[i][0])
    {
        FS_HANDLE file;
        file_name[length] = 0;
        kal_wstrcat(file_name,(const WCHAR *)suffix[i]);
        file = FS_Open(file_name,FS_READ_ONLY|FS_OPEN_NO_DIR);
        if (file >= 0)
        {
            FS_Close(file);
            kal_wstrcpy(full_path, file_name);
            pos = i+1;
            goto ret;
        }
        i++;
...

    {
    	...
    	for(i = 0; i < smcount; i++)
    	{
    		vm_ascii_to_ucs2((VMWSTR)file_name, 64, (VMSTR)mre_buildin_vsm_array[i].file_name);
    		if(kal_wstrcmp(file_name, short_name) == 0)
    		{
    			kal_wsprintf(full_path, "%c:\\%u.rom", 64, mre_buildin_vsm_array[i].address);
    			pos = 4;
                goto ret;
    		}
    	}
...
ret:
    ...
    MMI_PRINT(MOD_MRE,MMI_MRE_TRC_G8_PRO,"[MRE][PRO]vm_get_full_path_by_short_name pos = %d", pos);
    return 0;
}
