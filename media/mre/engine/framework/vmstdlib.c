...
VMINT vm_is_nfb_file(VMWSTR filename)
{
	if ((filename == NULL) || (vm_wstrlen(filename) == 0)) 
	{
		return 0;
	}
...
VMINT8 vm_get_exec_cid(VMWSTR wfilename)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (vm_parse_exec_format(wfilename) == VM_EXEC_NFB)
    {
