...
static mtv_result_t atv_player_start_video(atv_play_ctrl_t* self, media_config_video_struct* video_config)
{
    ...
    self->player_buffer_temp = med_alloc_asm_mem(self->app_id, 320*240*2);
#endif
#endif
    ...
    CalMdpPreviewPara.FrameBuffAddrTemp = (kal_uint32)self->player_buffer_temp;
...
#if defined(MT6260)
#if (LCD_HEIGHT > 480) || (LCD_WIDTH > 480)
        if (self->player_buffer_temp)
        {
            med_free_asm_mem(self->app_id, (void**)&self->player_buffer_temp);
        }
#endif
#endif
...
static mtv_result_t atv_rec_start_preview(atv_play_ctrl_t* self, media_config_video_struct* video_config)
{
    ...
    cal_mdp_video_data.FrameBuffAddrTemp = (kal_uint32)self->player_buffer_temp;
