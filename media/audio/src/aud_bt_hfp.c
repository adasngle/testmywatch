...
kal_bool aud_bt_hfp_sco_connect_req(void)
{
    ...
    if (aud_bt_hfp_ctx.next_cmd == AUD_BT_HFP_CMD_NONE)
    {
        if (aud_bt_hfp_ctx.state == AUD_BT_HFP_STATE_IDLE)
        {
			...
			{
                bt_hfadp_send_sco_connect_req(MOD_MED, aud_bt_hfp_ctx.connect_id);
			}
			else
...
void aud_bt_hfp_sco_connect_cnf_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (msg_p->result == BT_HFG_RESULT_OK)
    {
        ...
        aud_send_bt_audio_open_ind(aud_bt_hfp_ctx.src_mod_id);
        ...
        else
        {
            ...
            //kal_prompt_trace(MOD_MED,"aud_bt_hfp_set_speech_path_on, first_time_SCO=[%d]", aud_bt_hfp_ctx.first_time_SCO);

            /* Play a short mute tone to clear data in audio buffer to avoid noise in eSCO retransmission */
//            if(aud_bt_hfp_ctx.first_time_SCO)
            #ifdef __BT_DIALER_SUPPORT__
			if(!(AUD_BT_HFP_IS_BT_DIALER_SCO(aud_bt_hfp_ctx.sco_mode)))
            #endif
            {
    ...
    else
    {
        ...
        if (next_disc)
        {
            aud_send_bt_audio_close_ind(aud_bt_hfp_ctx.src_mod_id, MED_RES_OK);
        }
...
void aud_bt_hfp_bt_dialer_sco_ind_timer_hdlr(void *arg)
{
	...
	{
		bt_hfadp_send_sco_disconnect_req(MOD_MED, aud_bt_hfp_ctx.connect_id);
...
void aud_bt_hfp_sco_connect_ind_hdlr(ilm_struct *ilm_ptr)
{
	...
	#ifdef __BT_DIALER_SUPPORT__
	if(ilm_ptr->msg_id == MSG_ID_BT_HF_SCO_CONNECT_IND)
	{
...
void aud_bt_hfp_sco_disconnect_cnf_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_send_bt_audio_close_ind(aud_bt_hfp_ctx.src_mod_id, result);
...
void aud_bt_hfp_sco_disconnect_ind_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_send_bt_audio_close_ind(aud_bt_hfp_ctx.src_mod_id, MED_RES_HFP_CLOSE);
