...
void cam_free_working_buffer(void)
{
    ...
    if (cam_context_p->frame_buffer_temp_p)
    {
        med_free_asm_mem(cam_context_p->app_id, (void **)&cam_context_p->frame_buffer_temp_p);
        cam_context_p->frame_buffer_size = 0;
    }
#endif
...
void cam_preview_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (CAM_IN_STATE(CAM_READY))
    {
    ...
    #endif
        {
          ...
          #if (LCD_HEIGHT > 480) || (LCD_WIDTH > 480)
            {
                ...
                cam_context_p->frame_buffer_temp_p = (kal_uint32) med_alloc_asm_mem_framebuffer(cam_context_p->app_id, (CamGetPreviewImageInPara.ImageWidth*CamGetPreviewImageInPara.ImageHeight)<<1);
    //            cam_context_p->frame_buffer_temp_p = (kal_uint32) med_alloc_asm_mem_framebuffer(cam_context_p->app_id, (800*600)<<1); 
                CAM_PREVIEW_CHECK_IS_BUFFER_AVAILABLE(cam_context_p->frame_buffer_temp_p);		  
            }
          #endif
          #endif
        }        
	...
	CalMdpPreviewPara.FrameBuffAddrTemp = cam_context_p->frame_buffer_temp_p;
