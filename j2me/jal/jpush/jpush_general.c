    ...
    kal_bool delete_found_record)
{
    ...
    while (scan_entry_base + entry_size < file_size)
    {
        ...
        if (scan_pos == nfcurl_len)
        {
            ...
            /* Added when do ram optimize, but it cause a bug that if only one record exists in ppush.txt, the record will
              never be deleted as the copy_size is zero.
            it must be removed and change another modification.
            if(copy_size==0)
            			continue;
            */
            if (copy_size > 0)
