...
ifeq ($(words $(AUTO_REMOVE_CODE)),1)
  $(shell if exist $(strip $(DIS_DEF_FILE)) (del /Q $(strip $(DIS_DEF_FILE))))
  $(shell if not exist $(strip $(DIS_DEF_FILE)) (perl mtk_tools\Auto_RemovingCode.pl $(strip $(CUSTOMER)) $(PROJECT) $(CC) $(MTK_SUBSIDIARY)))
else
...
ifneq ($(strip $(CUSTOM_RELEASE)),TRUE)
	@if exist $(FIXPATH)\gps\core\custom_core\mt3318_gps_custom_core.bin \
		(if not exist "$(strip $(RELEASE_DIR))"\gps\core \
		md "$(strip $(RELEASE_DIR))"\gps\core) & \
		(copy /y $(FIXPATH)\gps\core\custom_core\mt3318_gps_custom_core.bin "$(strip $(RELEASE_DIR))"\gps\core\mt3318_gps_core.bin )
	@if exist "$(strip $(RELEASE_DIR))"\gps\core\custom_core \
		(rd /s /q "$(strip $(RELEASE_DIR))"\gps\core\custom_core)
else
	@if exist $(FIXPATH)\gps\core\mt3318_gps_core.bin \
		(if not exist "$(strip $(RELEASE_DIR))"\gps\core \
		md "$(strip $(RELEASE_DIR))"\gps\core) & \
		(copy /y $(FIXPATH)\gps\core\mt3318_gps_core.bin "$(strip $(RELEASE_DIR))"\gps\core\mt3318_gps_core.bin )
endif
endif
endif
